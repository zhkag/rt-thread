...
_internal_rw struct imx_sddev _s_sdcard_device = {
    .name = "sd0",
...
static rt_err_t sdcard_init_clock( struct imx_sddev *sddev )
{
    RT_ASSERT(RT_NULL != sddev);

    if (IMX6ULL_USDHC1_BASE == sddev->periph.paddr)
    {
        CLOCK_SetDiv(kCLOCK_Usdhc1Div, 0U);
        CLOCK_EnableClock(kCLOCK_Usdhc1);
    } else if (IMX6ULL_USDHC2_BASE == sddev->periph.paddr) {
...
static void _sdcard_gpio_init( struct imx_sddev *sddev )
{
    RT_ASSERT(RT_NULL != sddev);

    for (int i=0; i<SDCARD_CONTROL_PIN_NUM; i++)
    {
        imx6ull_gpio_init(&sddev->gpio[i]);
    }
}

static rt_err_t _sdcard_device_init( struct imx_sddev *sddev )
{
